...
enum debug_info_trace_ir_mapping_status copy_trace_content(const bt_trace *in_trace,
                                                           bt_trace *out_trace,
                                                           ...
                                                           bt_self_component *self_comp)
{
    ...
    if (graph_mip_version == 1) {
        ...
        if (trace_uid) {
            bt_trace_set_uid_status set_uid_status = bt_trace_set_uid(out_trace, trace_uid);
    ...
    for (i = 0; i < env_field_count; i++) {
        ...
        if (bt_value_is_signed_integer(value)) {
            set_env_status = bt_trace_set_environment_entry_integer(
                out_trace, value_name, bt_value_integer_signed_get(value));
        } else if (bt_value_is_string(value)) {
            set_env_status = bt_trace_set_environment_entry_string(out_trace, value_name,
    ...
    BT_COMP_LOGD("Copied content of trace: in-t-addr=%p, out-t-addr=%p", in_trace, out_trace);
...
enum debug_info_trace_ir_mapping_status copy_stream_content(const bt_stream *in_stream,
                                                            ...
                                                            bt_self_component *self_comp)
{
    ...
    BT_COMP_LOGD("Copying content of stream: in-s-addr=%p, out-s-addr=%p", in_stream, out_stream);
     ...
     * point.
     */
    bt_stream_set_user_attributes(out_stream, bt_stream_borrow_user_attributes_const(in_stream));

    BT_COMP_LOGD("Copied content of stream: in-s-addr=%p, out-s-addr=%p", in_stream, out_stream);
    status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK;
end:
    return status;
}

enum debug_info_trace_ir_mapping_status copy_packet_content(const bt_packet *in_packet,
                                                            ...
                                                            bt_self_component *self_comp)
{
    ...
    BT_COMP_LOGD("Copying content of packet: in-p-addr=%p, out-p-addr=%p", in_packet, out_packet);
    ...
    if (in_context_field) {
        ...
        status = copy_field_content(in_context_field, out_context_field, log_level, self_comp);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                      "Cannot copy context field: "
    ...
    BT_COMP_LOGD("Copied content of packet: in-p-addr=%p, out-p-addr=%p", in_packet, out_packet);
    status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK;
end:
    return status;
}

enum debug_info_trace_ir_mapping_status copy_event_content(const bt_event *in_event,
                                                           bt_event *out_event,
                                                           bt_logging_level log_level,
                                                           bt_self_component *self_comp)
{
    enum debug_info_trace_ir_mapping_status status;
    const bt_field *in_common_ctx_field, *in_specific_ctx_field, *in_payload_field;
    bt_field *out_common_ctx_field, *out_specific_ctx_field, *out_payload_field;

    BT_COMP_LOGD("Copying content of event: in-e-addr=%p, out-e-addr=%p", in_event, out_event);
    ...
    if (in_common_ctx_field) {
        ...
        status =
            copy_field_content(in_common_ctx_field, out_common_ctx_field, log_level, self_comp);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                      "Cannot copy common context field: "
    ...
    in_specific_ctx_field = bt_event_borrow_specific_context_field_const(in_event);
    if (in_specific_ctx_field) {
        out_specific_ctx_field = bt_event_borrow_specific_context_field(out_event);
        BT_ASSERT_DBG(out_specific_ctx_field);

        status =
            copy_field_content(in_specific_ctx_field, out_specific_ctx_field, log_level, self_comp);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                      "Cannot copy specific context field: "
    ...
    if (in_payload_field) {
        ...
        status = copy_field_content(in_payload_field, out_payload_field, log_level, self_comp);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                      "Cannot copy payloat field: "
    ...
    BT_COMP_LOGD("Copied content of event: in-e-addr=%p, out-e-addr=%p", in_event, out_event);
    status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK;
end:
    return status;
}

enum debug_info_trace_ir_mapping_status copy_field_content(const bt_field *in_field,
                                                           bt_field *out_field,
                                                           bt_logging_level log_level,
                                                           bt_self_component *self_comp)
{
    ...
    if (in_fc_type == BT_FIELD_CLASS_TYPE_BOOL) {
    ...
    } else if (bt_field_class_type_is(in_fc_type, BT_FIELD_CLASS_TYPE_UNSIGNED_INTEGER)) {
        bt_field_integer_unsigned_set_value(out_field,
                                            bt_field_integer_unsigned_get_value(in_field));
    } else if (bt_field_class_type_is(in_fc_type, BT_FIELD_CLASS_TYPE_SIGNED_INTEGER)) {
        bt_field_integer_signed_set_value(out_field, bt_field_integer_signed_get_value(in_field));
    } else if (in_fc_type == BT_FIELD_CLASS_TYPE_SINGLE_PRECISION_REAL) {
        bt_field_real_single_precision_set_value(
            out_field, bt_field_real_single_precision_get_value(in_field));
    } else if (in_fc_type == BT_FIELD_CLASS_TYPE_DOUBLE_PRECISION_REAL) {
        bt_field_real_double_precision_set_value(
            out_field, bt_field_real_double_precision_get_value(in_field));
        ...
        if (set_value_status != BT_FIELD_STRING_SET_VALUE_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                      "Cannot set string field's value: "
        ...
        nb_member_struct = bt_field_class_structure_get_member_count(in_field_class);
        ...
        for (i = 0; i < nb_member_struct; i++) {
            const bt_field_class_structure_member *member =
                bt_field_class_structure_borrow_member_by_index_const(in_field_class, i);

            in_member_name = bt_field_class_structure_member_get_name(member);
            in_member_field =
                bt_field_structure_borrow_member_field_by_name_const(in_field, in_member_name);
            out_member_field =
                bt_field_structure_borrow_member_field_by_name(out_field, in_member_name);

            status = copy_field_content(in_member_field, out_member_field, log_level, self_comp);
            if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
                BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                          ...
                                          "member-name=\"%s\"",
                                          out_field, out_member_field, in_member_name);
                goto end;
            }
        }
    } else if (bt_field_class_type_is(in_fc_type, BT_FIELD_CLASS_TYPE_ARRAY)) {
        ...
        if (bt_field_class_type_is(in_fc_type, BT_FIELD_CLASS_TYPE_DYNAMIC_ARRAY)) {
            set_len_status = bt_field_array_dynamic_set_length(out_field, array_len);
            if (set_len_status != BT_FIELD_DYNAMIC_ARRAY_SET_LENGTH_STATUS_OK) {
                BT_COMP_LOGE_APPEND_CAUSE(self_comp,
        ...
        for (i = 0; i < array_len; i++) {
            in_element_field = bt_field_array_borrow_element_field_by_index_const(in_field, i);
            out_element_field = bt_field_array_borrow_element_field_by_index(out_field, i);
            status = copy_field_content(in_element_field, out_element_field, log_level, self_comp);
            if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
                BT_COMP_LOGE_APPEND_CAUSE(self_comp,
        ...
        if (in_option_field) {
            ...
            out_option_field = bt_field_option_borrow_field(out_field);
            BT_ASSERT_DBG(out_option_field);
            status = copy_field_content(in_option_field, out_option_field, log_level, self_comp);
            if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
                BT_COMP_LOGE_APPEND_CAUSE(self_comp,
        ...
        in_selected_option_idx = bt_field_variant_get_selected_option_index(in_field);
        sel_opt_status = bt_field_variant_select_option_by_index(out_field, in_selected_option_idx);
        if (sel_opt_status != BT_FIELD_VARIANT_SELECT_OPTION_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
    ...
    BT_COMP_LOGT("Copied content of field: in-f-addr=%p, out-f-addr=%p", in_field, out_field);
