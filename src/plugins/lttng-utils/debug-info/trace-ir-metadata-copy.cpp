                                ...
                                bt_self_component *self_comp)
{
     ...
     *
     * • CASE 1: When the origin is Unix epoch (when "is unix epoch" is
     *   true)
     *
     * • CASE 2: When the origin is unknown (when "is unix epoch" is
     *   false)
     *
     * When the origin is custom, the origin is temporarily incorrectly
     * set to unknown. Below, it is corrected to the proper value.
     */
    ...
    if (graph_mip_version == 1) {
        /* Check whether the origin is custom. */
        const char *origin_uid = bt_clock_class_get_origin_uid(in_clock_class);

        if (origin_uid) {
            /*
             * CASE 3: When the origin is custom
             *
             * If the copies' clock class origin was incorrectly set as
             * "unknown" in the call to
             * `bt_clock_class_set_origin_is_unix_epoch`, the following
             * section overrides it with the correct custom origin.
             */
            ...
            if (set_origin_status != BT_CLOCK_CLASS_SET_ORIGIN_STATUS_OK) {
                BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                          ...
                                          "cc-origin-UID=\"%s\"",
                                          out_clock_class, origin_namespace ?: "none", origin_name,
                                          origin_uid);
