...
static
enum debug_info_trace_ir_mapping_status copy_clock_class_content_origin(
		...
		bt_logging_level log_level, bt_self_component *self_comp)
{
	...
	/*
	 * Calling `bt_clock_class_set_origin_is_unix_epoch` with the
	 * output of `bt_clock_class_origin_is_unix_epoch` handles both:
	 *
	 * • When the origin is Unix epoch (when "is unix epoch" is
	 *   true)
	 *
	 * • When the origin is unknown (when "is unix epoch" is false)
	 */
	...
	if (graph_mip_version == 1) {
		/*
		 * Check whether the origin is user-defined.
		 *
		 * If the copies' clock class origin was incorrectly set
		 * as "unknown" in the previous call, the following
		 * section will override it with the correct origin.
		 */
		const char *origin_namespace =
			bt_clock_class_get_origin_namespace(in_clock_class);

		const char *origin_name = bt_clock_class_get_origin_name(in_clock_class);

		const char *origin_uid = bt_clock_class_get_origin_uid(in_clock_class);

		if (origin_namespace && origin_name && origin_uid) {
			enum bt_clock_class_set_origin_status set_origin_status =
				bt_clock_class_set_origin(out_clock_class, origin_namespace,
					origin_name, origin_uid);
			...
			if (set_origin_status != BT_CLOCK_CLASS_SET_ORIGIN_STATUS_OK) {
				BT_COMP_LOGE_APPEND_CAUSE(self_comp,
					...
					"cc-origin-uid=%s", out_clock_class, origin_namespace,
					origin_name, origin_uid);
				return (int) set_origin_status;
			}
		}
	}
		...
		bt_self_component *self_comp)
{
	...
	status = copy_clock_class_content_origin(in_clock_class, out_clock_class,
		log_level, self_comp);
		...
		if (uid) {
			...
			if (set_uid_status != BT_CLOCK_CLASS_SET_UID_STATUS_OK) {
				BT_COMP_LOGE_APPEND_CAUSE(self_comp,
					...
					"cc-addr=%p, cc-uid=%s", out_clock_class,
					uid);
		...
		if (in_namespace) {
			...
			if (set_namespace_status != BT_STREAM_CLASS_SET_NAMESPACE_STATUS_OK) {
				BT_COMP_LOGE_APPEND_CAUSE(self_comp,
					...
					"out-sc-addr=%p, namespace=%s", out_stream_class,
					in_namespace);
				status = (int) set_namespace_status;
				goto end;
			}
		...
		if (in_uid) {
			...
			if (set_uid_status != BT_STREAM_CLASS_SET_UID_STATUS_OK) {
				BT_COMP_LOGE_APPEND_CAUSE(self_comp,
					...
					"out-sc-addr=%p, uid=%s", out_stream_class,
					in_uid);
				status = (int) set_uid_status;
				goto end;
			}
	...
	if (graph_mip_version == 1) {
		...
		if (in_event_class_namespace) {
			enum bt_event_class_set_namespace_status set_namespace_status =
				bt_event_class_set_namespace(out_event_class,
					in_event_class_namespace);

			if (set_namespace_status != BT_EVENT_CLASS_SET_NAMESPACE_STATUS_OK) {
				BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Error setting event class' "
				"namespace: ec-addr=%p, ec-namespace=%s", out_event_class,
				in_event_class_namespace);
				status = (int) set_namespace_status;
				goto end;
			}
		...
		/* Copy event class uid. */
		const char *in_event_class_uid = bt_event_class_get_uid(in_event_class);

		if (in_event_class_uid) {
			enum bt_event_class_set_uid_status set_uid_status =
				bt_event_class_set_uid(out_event_class, in_event_class_uid);

			if (set_uid_status != BT_EVENT_CLASS_SET_UID_STATUS_OK) {
				BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Error setting event class' "
				"uid: ec-addr=%p, ec-uid=%s", out_event_class,
				in_event_class_uid);
				status = (int) set_uid_status;
				goto end;
			}
